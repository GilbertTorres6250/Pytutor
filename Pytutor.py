...
def add_navigation_buttons():
    ...
    total_pages = (total_lessons + 12 - 1) // 12

    if current_page > 0:
        prev_button.place(x=20, y=550)
    ...
    if current_page < total_pages - 1:
        next_button.place(x=1150, y=550)
...
def previous_page(event=None):
    ...
    if current_page > 0:
        current_page -= 1
        update_lesson_list(1)

def next_page(event=None):
    global current_page
    if next_button['state'] != 'disabled':
        current_page += 1
        update_lesson_list(1)
...
next_button = Button(win, text="-->",font="impact", height=4, width=10,relief=RAISED, bd=5, command=next_page)
next_button.place_forget()
next_button["state"] = "normal"
prev_button = Button(win, text="<--",font="impact", height=4, width=10,relief=RAISED, bd=5,command=previous_page)
