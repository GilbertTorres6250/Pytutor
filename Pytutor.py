...
def update_lesson_list(course_id):
    ...
    for lesson in lessons:
        ...
        lesson_button = Button(lesson_frame, text=button_text, command=button_command, width=30, padx=20, pady=20,font="Arial", relief=RAISED, bd=5, bg=button_color, fg=fore)
...
def update_courses_list(Lesson_plans=None):
    ...
    for lessons in Lesson_plans:
        ...
        course_button = Button(frame, text=lessons_name, command=lambda l=lessons: display_lessons(*l), width=30,padx=20, pady=20, font=20, relief=RAISED, bd=5, bg=f, fg=b, activebackground=b,activeforeground=f)
...
def setColor(b_color, f_color):
    ...
    change()
def change():
    for window in [win, displayWindow, quizWindow, menuWindow]:
